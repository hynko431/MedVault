...
*(For production environments, use the 3-node HA cluster by running `docker compose -f docker/docker-compose.prod.yml up -d`)*
...
### Production Architecture (ILM & Aliases)

The service uses an enterprise-grade Index Lifecycle Management (ILM) and Rollover Alias architecture for zero-downtime scaling.

**Initialize Production ILM & Alias Architecture:**

```bash
# Run the setup script to create ILM policies, index templates, and initial write aliases
python scripts/setup_prod_es_ilm.py
```

**Run data migration script (Zero-Downtime Blue/Green):**

```bash
# Transfers existing data to the new Alias-backed indexes seamlessly
python scripts/migrate_alias_strategy.py
```

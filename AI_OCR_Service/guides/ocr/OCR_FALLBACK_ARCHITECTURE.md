...
### **Tier 3: PaddleOCR-VL (Tertiary/Fallback)**
...
**Requirements:**
```bash
# Optional - only needed for PaddleOCR-VL fallback
pip install transformers torch pillow
```

**Characteristics:**
- Model: Microsoft's `PaddleOCR-VL-base-handwritten`
...
└── PaddleOCR-VL_service.py          # Tier 3 provider
...
def extract_text_with_fallback(image_bytes: bytes) -> str:
    """
    Tries providers in order: Gemini → Google Vision → PaddleOCR-VL
...
PaddleOCR-VL: Missing dependencies for PaddleOCR-VL: No module named 'transformers'
```
...
# Optional: PaddleOCR-VL doesn't need config, but requires packages
...
```bash
# Install base requirements (includes PaddleOCR-VL optional deps)
...
- [ ] `transformers`, `torch`, `pillow` are installed (for PaddleOCR-VL)
...
| Aspect | OCR Fallback | LLM Fallback |
|--------|------------|------------|
| **Order** | Gemini → Google Vision → PaddleOCR-VL | Anthropic → OpenRouter → Groq |
| **Timeout Handling** | With retry backoff | Basic timeout only |
| **Local Fallback** | PaddleOCR-VL (on-device) | None (all cloud-based) |
...
**Issue: "PaddleOCR-VL model download timeout"**
```bash
# Solution: Pre-download model or disable PaddleOCR-VL in .env
pip install transformers torch pillow
python -c "from transformers import PaddleOCR-VLProcessor; PaddleOCR-VLProcessor.from_pretrained('microsoft/PaddleOCR-VL-base-handwritten')"
```
...
from app.services.PaddleOCR-VL_service import extract_text_with_PaddleOCR-VL
...
- [PaddleOCR-VL Model](https://huggingface.co/microsoft/PaddleOCR-VL-base-handwritten)
